...
## 🏆 v13: две «грандиозные» + поиск материалов

1. **🎟 Абонементы (пакеты занятий)** — продаёте пакет ученику
   (карточка ученика → «🎟 Абонемент», вводите `8;7200`). Баланс виден
   вам и ученику; после каждого проведённого занятия бот сам списывает
   одно и пишет ученику, сколько осталось.
2. **🔄 Перенос занятия учеником** — в «Мои занятия» → занятие →
   «Перенести» → выбор свободного слота. Старая запись отменяется,
   слот освобождается, вы получаете уведомление.
3. **🔎 Поиск материалов в интернете** (меню «📂 Материалы» → «Найти в
   интернете»): ученик или вы пишете название книги/статьи — бот ищет в
   открытых базах **arXiv** (научные статьи — PDF приходит файлом) и
   **Open Library** (книги — карточка, обложка, ссылка, пометка, если
   полный текст бесплатен). Без ключей API, легально.
